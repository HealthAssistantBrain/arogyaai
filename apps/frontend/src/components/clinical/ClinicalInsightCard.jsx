...
const Section = ({ icon, title, children }) => {
  const iconNode = createElement(icon, { size: 16, className: 'text-text-muted dark:text-slate-500' });

  return (
    <section className="pt-1">
      <div className="mb-3 flex items-center gap-2">
        {iconNode}
        <h3 className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-500 dark:text-text-muted">
          {title}
        </h3>
      </div>
      {children}
...
const BulletList = ({ items, emptyText }) => (
  items.length > 0 ? (
    <ul className="space-y-2 text-sm font-medium leading-relaxed text-slate-700 dark:text-text-secondary">
      {items.map((item) => (
    ...
    <p className="text-sm font-medium leading-relaxed text-slate-500 dark:text-text-muted">{emptyText}</p>
  )
);
...
const ClinicalInsightCard = ({ card, fallback, className = '' }) => {
  ...
  return (
    <article className={`rounded-lg border border-slate-200 bg-white p-6 shadow-sm dark:border-stroke dark:bg-[#1a1433] ${className}`}>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.18em] text-text-muted dark:text-slate-500">
            Clinical Condition
          </p>
          <h2 className="mt-2 text-2xl font-black tracking-tight text-text-primary dark:text-text-primary">
            {normalized.condition}
            <span className="ml-2 align-middle text-sm font-black text-text-muted dark:text-slate-500">
              {normalized.icdCode}
            </span>
          </h2>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <span className="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-slate-700 ring-1 ring-slate-200 dark:bg-white/5 dark:text-text-primary dark:ring-white/10">
      ...
      <div className="mt-6">
        <div className="mb-2 flex items-center justify-between text-[10px] font-black uppercase tracking-[0.18em] text-text-muted dark:text-slate-500">
      ...
      <div className="mt-6 space-y-5">
        <Section icon={Stethoscope} title="Clinical Insight">
          <p className="text-sm font-semibold leading-relaxed text-slate-700 dark:text-text-primary">
        ...
        <Section icon={BookOpen} title="References">
          <div className="flex flex-wrap gap-2">
            {references.map((reference) => (
              <span
                key={reference}
                className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-bold text-slate-600 dark:border-stroke dark:bg-white/[0.04] dark:text-text-secondary"
              >
                <ShieldCheck size={12} />
                {reference}
              </span>
