...
export default function Sidebar() {
    ...
    return (
        <aside className="w-[260px] bg-white dark:bg-card border-r border-slate-100 dark:border-stroke/50 flex flex-col h-screen sticky top-0 z-30 shrink-0 hidden lg:flex">
            {/* Logo / Brand */}
            <div
            ...
            >
                ...
                <div>
                    ...
                    <p className="text-[9px] text-text-muted font-bold uppercase tracking-[0.2em] mt-1 leading-none">
                        Healthcare AI
                    </p>
                </div>
            </div>
            ...
            <nav className="flex-1 px-4 overflow-y-auto custom-scrollbar">
                {navConfig.map((group, gIdx) => (
                    <div key={gIdx} className={gIdx > 0 ? 'mt-6 pt-6 border-t border-slate-100 dark:border-stroke/50' : ''}>
                        {group.section && (
                            <div className="text-[9px] font-bold text-text-muted uppercase tracking-[0.25em] px-3 mb-3 leading-none">
                                {group.section}
                            </div>
                        )}
                                    ...
                                    <NavLink
                                        ...
                                        className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-left ${
                                            isActive
                                                ...
                                                : 'text-slate-600 dark:text-text-muted hover:bg-slate-50 dark:hover:bg-white/5 hover:text-primary font-medium'
                                        }`}
                                    >
                                        {({ isActive }) => (
                                            <>
