...
const Signup = () => {
  ...
  return (
    <div className="bg-background dark:bg-card text-slate-900 dark:text-slate-100 min-h-screen flex items-center justify-center p-4 font-display relative overflow-hidden" style={{
      backgroundImage: `
      ...
      >
        ...
        <motion.div variants={itemVariants} className="hidden lg:flex flex-col justify-center space-y-8 p-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-text-primary dark:text-text-primary">ArogyaAI</h1>
          </div>

          <div className="space-y-4">
            <h2 className="text-5xl font-bold leading-tight text-text-primary dark:text-text-primary">
              Predictive health <br />
              <span className="text-primary">intelligence.</span>
            </h2>
            <p className="text-lg text-slate-600 dark:text-text-muted max-w-md">
              Master your biology with AI-driven longevity insights. Join thousands of early adopters optimizing their health span.
            </p>
          </div>

          <div className="relative rounded-xl overflow-hidden border border-stroke shadow-2xl bg-white/50 group">
            ...
            <div className="absolute bottom-4 left-4 right-4 p-4 rounded-lg flex items-center gap-4 bg-white/80 dark:bg-background/80 backdrop-blur-md border border-white/30 dark:border-stroke">
              <div className="size-10 rounded-full bg-primary/20 flex items-center justify-center text-primary">
                <BarChart3 size={20} />
              </div>
              <div>
                <p className="text-sm font-bold text-text-primary dark:text-text-primary">AI Longevity Score</p>
        ...
        <motion.div variants={itemVariants} className="w-full max-w-md mx-auto">
          <div className="bg-surface/50 p-8 lg:p-10 rounded-xl shadow-xl border border-slate-200 dark:border-stroke">

            <div className="lg:hidden flex items-center gap-2 mb-8">
              <div className="size-8 bg-primary rounded-lg flex items-center justify-center text-white">
                <Activity size={18} />
              </div>
              <span className="font-bold text-text-primary dark:text-text-primary">ArogyaAI</span>
            </div>

            <div className="mb-8">
              <h3 className="text-2xl font-bold text-text-primary dark:text-text-primary mb-2">Begin your longevity journey</h3>
              <p className="text-slate-500 dark:text-text-muted">Join 10,000+ early adopters mastering their biology.</p>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-2.5 px-4 border border-slate-200 dark:border-stroke rounded-lg hover:bg-slate-50 dark:hover:bg-card transition-colors group"
                >
                  ...
                  <span className="text-sm font-medium text-slate-700 dark:text-text-primary">Google</span>
                </button>
                <button
                  type="button"
                  ...
                  className="flex items-center justify-center gap-2 py-2.5 px-4 border border-slate-200 dark:border-stroke rounded-lg hover:bg-slate-50 dark:hover:bg-card transition-colors group"
                >
                  <Apple size={20} className="text-slate-700 dark:text-text-primary group-hover:text-primary transition-colors" />
                  <span className="text-sm font-medium text-slate-700 dark:text-text-primary">Apple</span>
                </button>
              </div>

              <div className="relative flex items-center py-2">
                <div className="flex-grow border-t border-slate-200 dark:border-stroke"></div>
                <span className="flex-shrink mx-4 text-text-muted text-xs uppercase tracking-widest font-semibold">Or with email</span>
                <div className="flex-grow border-t border-slate-200 dark:border-stroke"></div>
              </div>

              <form className="space-y-4" onSubmit={handleSubmit(onSubmit, handleValidationErrors)}>
                <div className="space-y-1.5">
                  <label className="text-sm font-semibold text-slate-700 dark:text-text-secondary ml-1">Full Name</label>
                  <input
                    {...register('fullName')}
                    className="w-full px-4 py-3 rounded-lg border-slate-200 dark:border-stroke bg-white dark:bg-background focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-primary transition-all outline-none text-slate-900 dark:text-slate-100 placeholder-slate-400"
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-semibold text-slate-700 dark:text-text-secondary ml-1">Email Address</label>
                  <input
                    {...register('email')}
                    className="w-full px-4 py-3 rounded-lg border-slate-200 dark:border-stroke bg-white dark:bg-background focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-primary transition-all outline-none text-slate-900 dark:text-slate-100 placeholder-slate-400"
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-semibold text-slate-700 dark:text-text-secondary ml-1">Password</label>
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-slate-600 focus:outline-none"
                      type="button"
                    >
                ...
                <div className="pt-2">
                  ...
                  <button
                    ...
                    className="w-full mt-2 py-4 bg-primary hover:bg-primary/90 text-white font-bold rounded-lg shadow-lg shadow-primary/30 transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2"
                    type="submit"
                  >
              ...
              <p className="text-center text-sm text-slate-500 mt-6 font-medium">
                ...
                <Link className="text-primary font-bold hover:underline ml-1" to={ROUTES.LOGIN}>Sign in</Link>
              </p>
            </div>

            <div className="mt-8 pt-6 border-t border-slate-100 dark:border-stroke flex items-center gap-2 justify-center">
              <ShieldCheck size={18} className="text-secondary" />
              <span className="text-[11px] text-text-muted uppercase tracking-widest font-bold">Secure HIPAA-compliant encryption</span>
            </div>
