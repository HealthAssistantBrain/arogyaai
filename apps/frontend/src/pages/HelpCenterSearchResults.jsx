...
const HelpCenterSearchResults = () => {
    ...
    const results = [
        ...
        },
        {
            ...
            typeColor: 'text-secondary bg-secondary/10',
            updated: '1 week ago',
                        ...
                        <div className="max-w-5xl mx-auto space-y-16">
                            ...
                            >
                                <div className="space-y-4">
                                    <h2 className="text-5xl lg:text-7xl font-black tracking-tighter text-text-primary dark:text-text-primary uppercase italic leading-[0.85]">Search Results</h2>
                                    <div className="flex items-center gap-3">
                                        <p className="text-[11px] font-black text-text-muted uppercase tracking-[0.25em] italic">Showing 12 results for</p>
                                        <span className="bg-primary/10 text-primary px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-widest border border-primary/20 shadow-sm italic">"{searchQuery}"</span>
                                    </div>
                                </div>

                                {/* Large Search Input Block */}
                                <div className="relative group/large">
                                    <div className="absolute inset-0 bg-primary/10 blur-3xl opacity-0 group-focus-within/large:opacity-50 transition-opacity duration-500"></div>
                                    <div className="relative flex items-center bg-surface backdrop-blur-3xl rounded-[2.5rem] p-4 shadow-[0_40px_80px_-20px_rgba(97,67,244,0.15)] ring-1 ring-[var(--color-primary)]/10 border border-white dark:border-stroke/50">
                                        <Search onClick={openCommandPalette} style={{ cursor: "pointer", pointerEvents: "auto" }} className="ml-8 text-primary" size={36} strokeWidth={3} />
                                        <input 
                                            className="flex-1 bg-transparent border-none outline-none px-8 py-6 text-2xl lg:text-3xl font-black uppercase tracking-widest text-text-primary dark:text-text-primary placeholder:text-text-secondary italic" 
                                            value={searchQuery}
                                        ...
                                        <button 
                                            ...
                                            className="mr-6 size-12 flex items-center justify-center rounded-2xl bg-slate-50 dark:bg-white/5 text-text-secondary hover:text-red-500 transition-all hover:scale-110 active:scale-90"
                                        >
                                    ...
                                    {filters.map((filter, idx) => (
                                        <button
                                            ...
                                            className={`px-8 py-5 rounded-2xl text-[11px] font-black uppercase tracking-widest whitespace-nowrap flex items-center gap-4 transition-all active:scale-95 shrink-0 ${
                                                filter.active 
                                                ? 'bg-card text-text-primary shadow-2xl shadow-[#13082a]/30 scale-105' 
                                                : 'bg-surface border border-slate-100 dark:border-stroke/50 text-slate-500 dark:text-text-muted hover:border-primary/30 hover:text-primary shadow-sm'
                                            }`}
                                        >
                                            <filter.icon size={18} strokeWidth={idx === 0 ? 3 : 2} className={filter.active ? 'text-primary' : 'opacity-60'} />
                            ...
                            <section className="space-y-8 pb-10">
                                {results.map((result, idx) => (
                                    <motion.div 
                                        ...
                                        className="group bg-surface p-10 rounded-[3rem] border border-transparent hover:border-primary/20 hover:shadow-2xl hover:shadow-primary/15 transition-all cursor-pointer relative overflow-hidden flex flex-col sm:flex-row gap-10"
                                    >
                                        <div className="flex-1 space-y-6">
                                            ...
                                            <h3 className="text-3xl font-black text-text-primary dark:text-text-primary group-hover:text-primary transition-colors uppercase tracking-tighter leading-tight italic decoration-primary/0 group-hover:decoration-primary/100 underline decoration-4 underline-offset-8 duration-500">{result.title}</h3>
                                            <p className="text-[12px] text-slate-500 dark:text-text-muted font-bold uppercase tracking-tight leading-relaxed max-w-3xl italic opacity-80 line-clamp-2">
                                                {result.desc.split(searchQuery).map((part, i, arr) => (
                                                    <React.Fragment key={i}>
                                                        {part}
                                                        {i < arr.length - 1 && (
                                                            <span className="text-primary underline decoration-2 underline-offset-4">{searchQuery}</span>
                                                        )}
                                                    </React.Fragment>
                                                ))}
                                            </p>
                                        </div>
                                        <div className="size-16 rounded-2xl bg-slate-50 dark:bg-white/5 flex items-center justify-center text-text-secondary group-hover:bg-primary group-hover:text-white group-hover:scale-110 group-hover:rotate-12 transition-all duration-500 self-center sm:self-start shadow-inner">
                            ...
                            {/* Bottom CTA Block */}
                            <section className="bg-gradient-to-br from-[#13082a] to-[#261B4D] dark:from-[#05040A] dark:to-[#131022] rounded-[3.5rem] p-16 text-text-primary relative overflow-hidden shadow-2xl shadow-[#13082a]/40 group/bottom">
                                <div className="absolute top-0 right-0 w-[40rem] h-[40rem] bg-primary/10 rounded-full blur-[140px] -translate-y-1/2 translate-x-1/2 group-hover/bottom:scale-110 transition-transform duration-1000"></div>
                                ...
                                <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-16">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center gap-6 w-full lg:w-auto">
                                        <button className="w-full sm:w-auto px-12 py-6 bg-white text-text-primary rounded-[1.75rem] font-black text-[11px] uppercase tracking-[0.25em] shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4 italic group/chat">
                                            <MessageSquare size={20} className="group-hover/chat:rotate-12 transition-transform" />
                                            Chat with Support
                                        </button>
                                        <button className="w-full sm:w-auto px-12 py-6 bg-primary text-white rounded-[1.75rem] font-black text-[11px] uppercase tracking-[0.25em] hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4 italic group/mail">
                            ...
                            <footer className="py-20 flex flex-col md:flex-row items-center justify-between gap-10 opacity-30 shrink-0">
                                ...
                                <div className="flex flex-wrap justify-center gap-10 text-[10px] font-black uppercase tracking-[0.25em] text-text-primary dark:text-text-primary">
                                    ...
                                    <button className="hover:text-primary transition-colors italic">Support Core</button>
