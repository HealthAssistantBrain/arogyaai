...
const Landing = () => {
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-[#EAEAEA] text-[#13082A] font-display selection:bg-[#6043F4]/30">
      
      {/* Top Navigation Bar */}
      <nav className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#6043F4] text-white">
              <Activity size={24} />
            </div>
            <span className="text-xl font-bold tracking-tight text-slate-900">ArogyaAI</span>
          </div>
          <div className="hidden md:flex items-center gap-10">
            <a className="text-sm font-medium text-slate-600 hover:text-[#6043F4] transition-colors" href="#features">Features</a>
            <a className="text-sm font-medium text-slate-600 hover:text-[#6043F4] transition-colors" href="#how-it-works">How it Works</a>
            <a className="text-sm font-medium text-slate-600 hover:text-[#6043F4] transition-colors" href="#integrations">Integrations</a>
            <a className="text-sm font-medium text-slate-600 hover:text-[#6043F4] transition-colors" href="#testimonials">Testimonials</a>
          </div>
          <div className="flex items-center gap-4">
            {/* ── Patch 1: Swap nav CTA based on auth state */}
            {isAuthenticated ? (
              <Link to={ROUTES.DASHBOARD} className="bg-[#6043F4] hover:bg-[#6043F4]/90 text-white px-6 py-2.5 rounded-lg text-sm font-bold shadow-lg shadow-[#6043F4]/20 transition-all active:scale-95">
                Go to Dashboard
              </Link>
            ) : (
              <>
                ...
                <Link to={ROUTES.SIGNUP} className="bg-[#6043F4] hover:bg-[#6043F4]/90 text-white px-6 py-2.5 rounded-lg text-sm font-bold shadow-lg shadow-[#6043F4]/20 transition-all active:scale-95">
                  Start Free Analysis
                </Link>
              </>
            )}
      ...
      <main className="flex-1">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-16">
              ...
              <div className="flex-1 space-y-8 text-center lg:text-left">
                <motion.div 
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-[#6043F4]/10 px-4 py-1.5 text-[#6043F4] text-sm font-semibold"
                >
                  <Sparkles size={14} />
                ...
                >
                  Master Your Longevity with <span className="text-[#6043F4]">Predictive Intelligence</span>
                </motion.h1>
                ...
                >
                  ...
                  {isAuthenticated ? (
                    <Link to={ROUTES.DASHBOARD} className="bg-[#6043F4] hover:bg-[#6043F4]/90 text-white px-8 py-4 rounded-xl text-lg font-bold shadow-xl shadow-[#6043F4]/20 transition-all flex items-center gap-2 active:scale-95">
                      Go to Dashboard
                      <ArrowRight size={20} />
                    </Link>
                  ) : (
                    <Link to={ROUTES.SIGNUP} className="bg-[#6043F4] hover:bg-[#6043F4]/90 text-white px-8 py-4 rounded-xl text-lg font-bold shadow-xl shadow-[#6043F4]/20 transition-all flex items-center gap-2 active:scale-95">
                      Start Free Analysis
                      <ArrowRight size={20} />
                    </Link>
                  )}
                  <button className="bg-white border border-slate-200 text-slate-900 hover:bg-slate-50 px-8 py-4 rounded-xl text-lg font-bold transition-all flex items-center gap-2 shadow-sm">
                    <PlayCircle size={24} className="text-[#009CDE]" />
                    Watch Demo
                  </button>
                </motion.div>
              </div>
              ...
              <div className="flex-1 relative w-full max-w-[600px]">
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    <Heart size={20} className="text-[#6043F4]" />
                  ...
                  <div className="mt-2 h-2 w-full bg-slate-200 rounded-full overflow-hidden">
                    <div className="h-full bg-[#009CDE] w-3/4"></div>
                  </div>
                </motion.div>
              </div>
            </div>
        ...
        <section className="py-24 bg-white">
          <div className="mx-auto max-w-7xl px-6 text-center">
            ...
            <div className="mx-auto max-w-5xl rounded-2xl bg-[#EAEAEA] p-4 shadow-inner">
              <motion.div 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                <motion.div 
                  key={idx}
                  whileHover={{ y: -5 }}
                  className="bg-white p-8 rounded-xl border border-slate-200 hover:border-[#6043F4]/30 transition-all hover:shadow-xl group"
                >
                  <div className={`mb-6 inline-flex h-12 w-12 items-center justify-center rounded-lg ${feature.color === 'primary' ? 'bg-[#6043F4]/10 text-[#6043F4] group-hover:bg-[#6043F4]' : 'bg-[#009CDE]/10 text-[#009CDE] group-hover:bg-[#009CDE]'} transition-colors group-hover:text-white`}>
        ...
        {/* Integrations Grid */}
        <section className="py-24 bg-slate-900 text-white overflow-hidden" id="integrations">
          <div className="mx-auto max-w-7xl px-6">
            <div className="text-center mb-16">
              <h2 className="text-3xl font-bold mb-4">Sync Your Ecosystem</h2>
              <p className="text-slate-400">Connect with the tools you already use to get the full picture.</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 opacity-70 grayscale hover:grayscale-0 transition-all duration-500">
              {[
                ...
                <div key={idx} className="flex items-center justify-center p-8 bg-slate-800/50 rounded-2xl border border-slate-700 hover:bg-slate-800 transition-colors group">
                  <item.icon size={36} className="text-white group-hover:text-[#009CDE] transition-colors" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 relative">
              ...
              {[
                ...
                <div key={idx} className="relative z-10 flex flex-col items-center text-center">
                  <div className="mb-8 flex h-20 w-20 items-center justify-center rounded-full bg-white shadow-xl text-[#6043F4] font-bold text-2xl border-4 border-slate-100">
                    {item.step}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <div key={idx} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
                  <div>
                    <div className="flex gap-1 text-[#009CDE] mb-4">
          ...
          <div className="mx-auto max-w-7xl px-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
              ...
              <div className="col-span-1 md:col-span-1">
                <div className="flex items-center gap-2 mb-6">
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#6043F4] text-white">
                    <Activity size={18} />
                  </div>
                  <span className="text-lg font-bold text-white tracking-tight">ArogyaAI</span>
                </div>
                <p className="text-sm leading-relaxed mb-6">
                  Pioneering the future of predictive health intelligence and human longevity.
                </p>
                <div className="flex gap-4">
                  <a className="h-10 w-10 flex items-center justify-center rounded-lg bg-slate-800 hover:bg-[#6043F4] transition-colors" href="#">
                    <Share2 size={16} />
                  </a>
                  <a className="h-10 w-10 flex items-center justify-center rounded-lg bg-slate-800 hover:bg-[#6043F4] transition-colors" href="#">
                    <Mail size={16} />
              ...
              <div>
                ...
                <ul className="space-y-4 text-sm font-medium">
                  <li><Link className="hover:text-white transition-colors" to={ROUTES.DASHBOARD}>Risk Dashboard</Link></li>
                  <li><a className="hover:text-white transition-colors" href="#">AI Analysis</a></li>
                  <li><a class="hover:text-white transition-colors" href="#integrations">Integrations</a></li>
              ...
              <div>
                ...
                <ul className="space-y-4 text-sm font-medium">
                  ...
                  <li><a class="hover:text-white transition-colors" href="#">Medical Board</a></li>
                </ul>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input className="bg-slate-800 border-none rounded-lg px-4 py-2 text-sm w-full focus:ring-1 focus:ring-[#6043F4] text-white" placeholder="Email" type="email" />
                  <button className="bg-[#6043F4] hover:bg-[#6043F4]/90 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all shrink-0">
                    Join
                  </button>
                </div>
              </div>
            ...
            <div className="border-t border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-medium">
              ...
              <div className="flex gap-8">
                <Link className="hover:text-white" to={ROUTES.PRIVACY}>Privacy Policy</Link>
                <Link className="hover:text-white" to={ROUTES.TERMS}>Terms of Service</Link>
